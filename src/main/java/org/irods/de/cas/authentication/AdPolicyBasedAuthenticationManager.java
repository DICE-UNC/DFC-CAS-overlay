 ...
 * under the License.
 */
package org.irods.de.cas.authentication;
...
public class AdPolicyBasedAuthenticationManager implements AuthenticationManager {
     ...
     */
    protected AdPolicyBasedAuthenticationManager() {}
     ...
     */
    public AdPolicyBasedAuthenticationManager(final AuthenticationHandler ... handlers) {
     ...
     */
    public AdPolicyBasedAuthenticationManager(final List<AuthenticationHandler> handlers) {
     ...
     */
    public AdPolicyBasedAuthenticationManager(final Map<AuthenticationHandler, PrincipalResolver> map) {
        ...
        for (final Credential credential : credentials) {
            ...
            for (final Map.Entry<AuthenticationHandler, PrincipalResolver> entry : this.handlerResolverMap.entrySet()) {
                ...
                if (handler.supports(credential)) {
                    ...
                    try {
                    	logger.info("DFC LOGGING: about to authAndResPrincipal, builder={}, credential={}", builder, credential);
                    	logger.info("DFC LOGGING: about to authAndResPrincipal, entry.getvalue={}, handler={}", entry.getValue(), handler);
                    	...
                    	logger.info("DFC LOGGING: just to see if we are here");
    ...
    private void authenticateAndResolvePrincipal(final AuthenticationBuilder builder, final Credential credential,
        ...
        logger.info("DFC LOGGING: In authenticateAndResolvePrincipal: credential={}", credential);
        final HandlerResult result = handler.authenticate(credential);
        logger.info("DFC LOGGING: In authenticateAndResolvePrincipal: after authenticate");
